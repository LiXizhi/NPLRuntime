    ...
    /* [in] */ VMR9PresentationInfo *lpPresInfo)
{
    ...
    // Maybe wha we can do here is to WaitOnSingleObject
    // with a specific timeout, and allow the CreateDevice and
    // then passed back to the Allocator from VideoFilterApp.
...
//            break;
    } // WaitOnSingleObject(hD3DCreated, 500) == WAIT_TIMEOUT); // check until it is signaled
    ...
    if( hr == D3DERR_DEVICELOST)
    {
        if (m_D3DDev->TestCooperativeLevel() == D3DERR_DEVICENOTRESET) 
        {
            ...
            // here we need to do something to somehow so we can restore
            // the D3D device and pass it back to us
...
//        FAIL_RET( m_scene.DrawScene(m_D3DDev, texture ) );
    ...
    // Maybe at this point we can release the mutex so we are ready to render stuff
