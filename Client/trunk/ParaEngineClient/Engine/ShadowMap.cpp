...
void CShadowMap::SetShadowMapTexelSize(int nWidth, int nHeight)
{
	if(m_shadowTexWidth!=nWidth || m_shadowTexHeight!=nHeight)
	{
		...
		if(m_pSMColorTexture!=0 || m_pSMZSurface!=0)
		{
...
bool CShadowMap::BuildPSMProjectionMatrix()
{
	...
	else
	{
		...
		ParaMatrixPerspectiveFovLH( &virtualCameraProj, D3DXToRadian(60.f), m_fAspect, m_zNear, m_zFar);
	}
...
HRESULT CShadowMap::SetShadowTexture(CEffectFile& pEffect, int nTextureIndex, int nUseBlur)
{
	...
	CGlobals::GetRenderDevice()->SetTextureStageState(nTextureIndex, D3DTSS_TEXCOORDINDEX, nTextureIndex);
	CGlobals::GetRenderDevice()->SetTextureStageState(nTextureIndex, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_PROJECTED);
	*/
...
{
	// CGlobals::GetRenderDevice()->SetTextureStageState(nTextureIndex, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_DISABLE);
