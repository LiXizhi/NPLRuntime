...
namespace ParaEngine
{
	...
	*/
	...
	public:
		...
		ATTRIBUTE_METHOD1(CGUIContainer, GetSelfPaintRenderTargetName_s, const char**) { *p1 = cls->GetSelfPaintRenderTargetName().c_str(); return S_OK; }
		ATTRIBUTE_METHOD1(CGUIContainer, SetSelfPaintRenderTargetName_s, const char*) { cls->SetSelfPaintRenderTargetName(p1); return S_OK; }
		...
		*/
		...
		/** set the name of the render target that this object will render into. if not set, it will be same as GetIdentifier() */
		void SetSelfPaintRenderTargetName(const char* name);
		const std::string& GetSelfPaintRenderTargetName();
	...
	protected:
		...
		std::string  m_strSelfPaintRenderTargetName;
