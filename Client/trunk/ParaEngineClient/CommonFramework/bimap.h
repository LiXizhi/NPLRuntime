 ...
 *       "Portions copyright 2002-2006 Joaquín M López Muñoz (joaquin@tid.es)"
 *
...
#define VERSION_BIMAP_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761 0x00010002
...
public:
  ...
  public:
    ...
    public:
      const_iterator(){}
// Modified by LiXizhi 2008.10.6 when porting from vs2003 to vs 2005
#if _MSC_VER >= 1400
      // this is Visual C++ 2005
      const_iterator(const typename prebimap::from_impl::iterator& it):fit(it.fit){}
#else
  ...
  friend class from::iterator;
  friend class from::const_iterator;
#else
...
// Modified by LiXizhi 2008.10.6 when porting from vs2003 to vs 2005
#if _MSC_VER >= 1400
      // this is Visual C++ 2005
      const_iterator(const typename prebimap::to_impl::iterator& it):tit(it.tit){}
#else
  ...
  friend class to::iterator;
  friend class to::const_iterator;
#else
  ...
  std::pair<typename from::iterator,typename to::iterator>
  insert(
    typename from::iterator fit,typename to::iterator tit,
    const typename from::value_type& x)
  {
    if(!from.adjacent(fit.fit,x.first)){
      ...
      if(fit!=fset.end()){ /* small chance x is already inserted */
        ...
        if(tnit!=tset.end()&&element_by_from(*(fit.fit))==element_by_to(*tnit)){
          return std::make_pair(fit,to::iterator(tnit));
        }
        else throw duplicate_value();
      }
    }
    ...
    return std::make_pair(from::iterator(fnit),to::iterator(tnit));
  }
...
public:
  ...
  iterator                 begin(){return from.begin();}
  const_iterator           begin()const{return from.begin();}
  iterator                 end(){return from.end();}
  const_iterator           end()const{return from.end();}
  reverse_iterator         rbegin(){return from.rbegin();}
  const_reverse_iterator   rbegin()const{return from.rbegin();}
  reverse_iterator         rend(){return from.rend();}
  const_reverse_iterator   rend()const{return from.rend();}
  size_type                size()const{return from.size();}
  size_type                max_size()const{return from.max_size();}
  bool                     empty()const{return from.empty();}
  allocator_type           get_allocator()const{return from.get_allocator();}
  from_binding_t           operator[](const key_type& key){return from[key];}
  const_from_binding_t     operator[](const key_type& key)const{return from[key];}
  using                    super::insert;
  std::pair<iterator,bool> insert(const value_type& x){return from.insert(x);}
  iterator                 insert(iterator it,const value_type& x){return from.insert(it,x);}
  template<
    typename it_type>
  void                     insert(it_type first,it_type last){from.insert(first,last);}

#ifdef _MSC_VER /* see note in from::erase */
  iterator                 erase(iterator it){return from.erase(it);}
#else
  void                     erase(iterator it){from.erase(it);}
#endif

  void                     erase(iterator first,iterator last){from.erase(first,last);}
  size_type                erase(const key_type& key){return from.erase(key);}
  void                     clear(){from.clear();}
  void                     swap(bimap_equal_types& x){from.swap(x.from);}
  friend void              swap(bimap_equal_types& x,bimap_equal_types& y){x.swap(y);}
  key_compare              key_comp()const{return from.key_comp();}
  value_compare            value_comp()const{return from.value_comp();}
  iterator                 find(const key_type& key){return from.find(key);}
  const_iterator           find(const key_type& key)const{return from.find(key);}
  size_type                count(const key_type& key)const{return from.count(key);}
  iterator                 lower_bound(const key_type& key){return from.lower_bound(key);}
  const_iterator           lower_bound(const key_type& key)const{return from.lower_bound(key);}
  iterator                 upper_bound(const key_type& key){return from.upper_bound(key);}
  const_iterator           upper_bound(const key_type& key)const{return from.upper_bound(key);}
  std::pair<
    iterator,
    iterator>              equal_range(const key_type& key){return from.equal_range(key);}
  std::pair<
    const_iterator,
    const_iterator>        equal_range(const key_type& key)const{return from.equal_range(key);}

  /* Promotion of unambiguous parts of the to memberspace */

  std::pair<
    to_iterator,
    bool>                  insert(const to_value_type& x){return to.insert(x);}
  to_iterator              insert(to_iterator it,const to_value_type& x){return to.insert(it,x);}
  void                     insert(const to_value_type *first,const to_value_type *last)
                           {to.insert(first,last);}

#ifdef _MSC_VER /* see note in from::erase */
  to_iterator              erase(to_iterator it){return to.erase(it);}
#else
  void                     erase(to_iterator it){to.erase(it);}
#endif

  void                     erase(to_iterator first,to_iterator last){to.erase(first,last);}
...
public:
  ...
  size_type                erase(const key_type& key){return from.erase(key);}
  void                     clear(){from.clear();}
  void                     swap(bimap_different_types& x){from.swap(x.from);}
  ...
  to_binding_t             operator[](const to_key_type& key){return to[key];}
  const_to_binding_t       operator[](const to_key_type& key)const{return to[key];}
  std::pair<
    to_iterator,
    bool>                  insert(const to_value_type& x){return to.insert(x);}
  to_iterator              insert(to_iterator it,const to_value_type& x){return to.insert(it,x);}
  ...
  void                     erase(to_iterator first,to_iterator last){to.erase(first,last);}
  size_type                erase(const to_key_type& key){return to.erase(key);}
  to_iterator              find(const to_key_type& key){return to.find(key);}
  const_to_iterator        find(const to_key_type& key)const{return to.find(key);}
  size_type                count(const to_key_type& key)const{return to.count(key);}
  to_iterator              lower_bound(const to_key_type& key){return to.lower_bound(key);}
  const_to_iterator        lower_bound(const to_key_type& key)const{return to.lower_bound(key);}
  to_iterator              upper_bound(const to_key_type& key){return to.upper_bound(key);}
  const_to_iterator        upper_bound(const to_key_type& key)const{return to.upper_bound(key);}
  std::pair<
    to_iterator,
    to_iterator>           equal_range(const to_key_type& key){return to.equal_range(key);}
  std::pair<
    const_to_iterator,
    const_to_iterator>     equal_range(const to_key_type& key)const{return to.equal_range(key);}
 ...
 */
  ...
  bool operator==(const bimap& r)const{return from==r.from;}
  bool operator!=(const bimap& r)const{return from!=r.from;}
  bool operator< (const bimap& r)const{return from<r.from;}
  bool operator> (const bimap& r)const{return from>r.from;}
  bool operator<=(const bimap& r)const{return from<=r.from;}
...
#elif VERSION_BIMAP_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761!=0x00010002
