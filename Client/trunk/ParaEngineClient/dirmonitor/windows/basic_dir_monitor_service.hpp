...
// added by LiXizhi: 2016.9.30
#if !defined(RECUSRIVE_DIRECTORY_MONITOR)
...
public:
    ...
    explicit basic_dir_monitor_service(boost::asio::io_context &io_context)
        ...
        async_monitor_work_(std::make_unique<boost::asio::executor_work_guard<boost::asio::io_context::executor_type>>(async_monitor_io_service_.get_executor())),
    ...
    void add_directory(implementation_type &impl, const std::string &dirname)
    {
		...
		helper::throw_system_error_if(FALSE == ReadDirectoryChangesW(ck_holder->handle, ck_holder->buffer, sizeof(ck_holder->buffer), RECUSRIVE_DIRECTORY_MONITOR, 0x1FF, &bytes_transferred, &ck_holder->overlapped, NULL),
...
private:
    ...
    void work()
    {
        ...
        if (impl) 
        {
			...
			helper::throw_system_error_if(!ReadDirectoryChangesW(ck_holder->handle, ck_holder->buffer, sizeof(ck_holder->buffer), RECUSRIVE_DIRECTORY_MONITOR, 0x1FF, &bytes_transferred, &ck_holder->overlapped, NULL),
