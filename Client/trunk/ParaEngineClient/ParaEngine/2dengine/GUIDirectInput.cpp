...
{
	GETD3D(CGlobals::GetRenderDevice())->SetCursorPosition(x, y, D3DCURSOR_IMMEDIATE_UPDATE);
}
...
void CDirectMouse::SetCursorFromFile(const char *szCursor, int XHotSpot, int YHotSpot)
{
	...
	if(pCursor)
	{
		...
		if (pCursorSurface) 
		{
			if(SUCCEEDED(hr = GETD3D(CGlobals::GetRenderDevice())->SetCursorProperties( m_XHotSpot,m_YHotSpot, pCursorSurface)))
			{
