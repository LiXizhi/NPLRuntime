...
namespace ParaScripting
{
	...
	*/
	...
	ParaUIObject ParaUI::Pick(const object& params)
	{
		using namespace luabind;
		int nType = type(params);
		if (nType != LUA_TTABLE)
		{
			OUTPUT_LOG("error: ParaUI::Pick() expects a table as input parameter\n");
			return ParaUIObject();
		}

		float rayX = object_cast<float>(params["rayX"]);
		float rayY = object_cast<float>(params["rayY"]);
		float rayZ = object_cast<float>(params["rayZ"]);
		float dirX = object_cast<float>(params["dirX"]);
		float dirY = object_cast<float>(params["dirY"]);
		float dirZ = object_cast<float>(params["dirZ"]);
		float fMaxDistance = object_cast<float>(params["maxDistance"]);
		const char* sFilterFunc = object_cast<const char*>(params["filterFunc"]);
		float fHitDist = -1.f;
		CGUIBase* pObj = CGlobals::GetGUI()->Pick(rayX, rayY, rayZ, dirX, dirY, dirZ, fMaxDistance, sFilterFunc, &fHitDist);
		params["hitDist"] = fHitDist;
		return ParaUIObject(pObj);
	}
