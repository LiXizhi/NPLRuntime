...
bool CMoviePlatform::ResizeImage(const string& filename, int width, int height, const string& destFilename)
{
	...
	HRESULT hr = pd3dDevice->CreateTextureFromFileEx(filename.c_str(), width, height, D3DX_FROM_FILE, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, (LPDIRECT3DTEXTURE9*)(&pTexture));
	if(SUCCEEDED(hr))
	{
