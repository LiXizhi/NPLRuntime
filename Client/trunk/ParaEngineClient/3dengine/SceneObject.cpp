	...
	*/
	...
	if(CGlobals::WillGenReport())
	{
		...
		snprintf(sMeshReport, 50, "F->B:%d B->F:%d(%d) char:%d(%d) post:%d", 
			sceneState.listPRSolidObject.size(), sceneState.listPRSmallObject.size(), nMeshRendered, sceneState.listPRBiped.size() + sceneState.listPRTransparentBiped.size(), nCharacterRendered, sceneState.listPRTransparentObject.size());
		sMeshReport[49] = '\0';
