...
namespace ParaEngine
{
    ...
    {
        int tex = pass.tex;
        TextureEntity* pTexture = nullptr;
        if (tex >= CParaXModel::MAX_MODEL_TEXTURES || paraXModel->specialTextures[tex] == -1 || paraXModel->replaceTextures[paraXModel->specialTextures[tex]] == 0) {
            pTexture = paraXModel->textures[tex].get();
        }
        else {
            pTexture = paraXModel->replaceTextures[paraXModel->specialTextures[tex]];
        }
        return ExportImage(asset_ptr<TextureEntity>(pTexture));
    }
    ...
    std::shared_ptr<GLTFImage> GltfModel::ExportImage(asset_ptr<TextureEntity> texture)
    {
        ...
        std::string filename = CParaFile::GetParentDirectoryFromPath(m_filename) + image->uri;
        ...
        if (texture)
        {
            ...
            {
                CParaFile file;
                if (file.OpenFile(filename.c_str(), true))
                {
                ...
                {
                    OUTPUT_LOG("GLTF exporter: failed to open file: %s", filename.c_str());
                }
            }
        }
        return image;
