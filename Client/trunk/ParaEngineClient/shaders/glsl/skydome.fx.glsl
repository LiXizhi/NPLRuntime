...
Unifroms
{
	...
	vec2 cloudOffset:cloudOffset;
	float skyDomeHeight:skyDomeHeight;
	vec3 lightSkyColor:lightSkyColor;
	vec3 darkSkyColor:darkSkyColor;
	vec3 sunColor:sunColor;
	vec3 cloudColor:cloudColor;
	vec2 cloudOffset:cloudOffset;
	float cloudBlendWeight:cloudBlendWeight;
	vec2 sunIntensity:sunIntensity;
	vec2 sunSize:sunSize;
	vec2 moonSize:moonSize;
...
#CODEBLOCK_END skyDomeEffect_vert
...
#CODEBLOCK_END skyDomeDayEffect_frag
...
#CODEBLOCK_END skyDomeNightEffect_frag
...
technique default
{
	pass P0
	{
		VertexShader = compile vs skyDomeEffect_vert();
		PixelShader = compile ps skyDomeDayEffect_frag();
	}
	pass P0
	{
		VertexShader = compile vs skyDomeEffect_vert();
		PixelShader = compile ps skyDomeNightEffect_frag();
	}	
}
