...
namespace darknet
{
    ...
    layers = -1, 8
    */
    RouteLayer::RouteLayer(Net *net, Layer *prev_layer, std::shared_ptr<Options> options) : Layer(net, prev_layer, options)
    {
        m_name = "route";
        m_type = s_route;
    ...
    void RouteLayer::Forward()
    {
        ...
        for (int i = 0; i < intput_layers_size; ++i)
        {
            ...
            auto layer    = GetIndexLayer(index);
