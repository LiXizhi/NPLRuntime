...
YoloLayer::YoloLayer(Net *net, Layer *prev_layer, Options *options) : Layer(net, prev_layer, options)
{
    m_name = "yolo";
    m_type = s_yolo;
...
void YoloLayer::Forward()
{
    ...
    auto max_truths          = GetNet()->GetMaxTruths();
    auto truths_size         = GetNet()->GetTruthsSize();
    auto max_truths_size     = GetNet()->GetMaxTruthsSize();
        ...
        for (int truth_index = 0; truth_index < max_truths; truth_index++)
        {
            ...
            *os << "x = " << truth.m_x << " y = " << truth.m_y << " w = " << truth.m_w << " h = " << truth.m_h << " classid = " << classid << std::endl;
            ...
            for (int mask = 0; mask < m_masks.size(); ++mask)
            {
                ...
                auto predict   = GetPredictBox(out_datas, GetAnchorsData(), m_masks[mask], box_index, grid_x, grid_y, m_out_width, m_out_height, GetNet()->GetWidth(), GetNet()->GetHeight(), m_out_width * m_out_height);
            ...
            auto predict   = GetPredictBox(out_datas, GetAnchorsData(), m_masks[best_mask], box_index, grid_x, grid_y, m_out_width, m_out_height, GetNet()->GetWidth(), GetNet()->GetHeight(), m_out_width * m_out_height);
        ...
        *os << "predicts => " << std::endl;
        ...
        for (int i = 0; i < stride; ++i)
        {
            ...
            for (int n = 0; n < m_boxes; ++n)
            {
                ...
                auto bbox = GetPredictBox(out_datas, GetAnchorsData(), m_masks[n], box_index, col, row, m_out_width, m_out_height, GetNet()->GetWidth(), GetNet()->GetHeight(), m_out_width * m_out_height);
