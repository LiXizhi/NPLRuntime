//-----------------------------------------------------------------------------
// Class:	MD5
// Authors:	based on a C implementation of the MD5 algorithm written by L. Peter Deutsch.
...
#undef SET
	...
	void MD5::feed( const char* data )
	{
		if(data!=0){
			feed((const unsigned char*)data, strlen(data));
		}
	}
	...
	const std::string MD5::hex(bool bLowercase)
	{
		if( !m_finished )
			finalize();
