...
#define PE_FAST_ATOF_RELAVANT_DECIMALS 15
...
inline const char* fast_atoreal_move( const char* c, Real& out, bool check_comma = true)
{
	...
	if (*c == '.' || (check_comma && c[0] == ',' && c[1] >= '0' && c[1] <= '9')) // allow for commas, too
	{
		...
		// number of digits to be read. PE_FAST_ATOF_RELAVANT_DECIMALS can be a value between
		// 1 and 15.
		unsigned int diff = PE_FAST_ATOF_RELAVANT_DECIMALS;
