...
namespace darknet
{
    ...
    */
    ...
    ShortCutLayer::ShortCutLayer(Net *net, Layer *prev_layer, std::shared_ptr<Options> options) : Layer(net, prev_layer, options)
    {
        m_name = "shortcut";
        m_type = s_shortcut;
        ...
        m_input_layer = from < 0 ? (from + GetLayerIndex(nullptr)) : from;
        if (GetIndexLayer(m_input_layer) == nullptr)
        {
            std::cerr << "shortcut layer index = " << GetLayerIndex(nullptr) << " invalid from = " << from << std::endl;
            m_out_width   = 0;
            m_out_height  = 0;
            m_out_filters = 0;
        }
    ...
    void ShortCutLayer::Forward()
    {
        ...
        auto batch_out_size = GetBatchOutSize();
        auto layer          = GetIndexLayer(m_input_layer);
