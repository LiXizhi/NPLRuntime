...
namespace darknet
{
    SoftMaxLayer::SoftMaxLayer(Net *net, Layer *prev_layer, std::shared_ptr<Options> options) : Layer(net, prev_layer, options)
    {
        m_name = "softmax";
        m_type = s_softmax;
    ...
    float SoftMaxLayer::Loss()
    {
        auto in_size = GetInSize();
        auto batch   = GetBatch();
        softmax_cross_entropy_cpu(batch, in_size, GetOutDatas(), GetInTruths(), GetInDeltas(), GetLossesData());
