...
使用现有 `emcmake cmake` 配置及 `cmake --build` 命令即可。源码必须完整包含本分支 `Server/trunk/mujoco-3.10.0` 和 `Server/trunk/onnxruntime-1.27.1`，工具链需满足依赖的 CMake 要求（当前至少 3.28）。已验证 Emscripten 3.1.59（Windows 宿主的 Release 构建、Node 原生冒烟和 Chrome MicroDuck 运行）及此前的 4.0.10；公司 Linux CI 的完整构建仍需实际验证。
...
旧版 Emscripten（例如 3.1.59）未声明 CMake 的 `WHOLE_ARCHIVE` 特性时，机器人构建补充对应链接参数，保留 MuJoCo 静态插件注册。已有 SDK 的特性定义保持不变。

自动 ONNX 构建需要在宿主机运行的 `protoc 3.21.12`。可通过 `-DONNX_CUSTOM_PROTOC_EXECUTABLE=<宿主protoc路径>` 指定；该路径会传给隔离的 ONNX 子工程。没有指定时，构建按本分支 ONNX 的依赖清单下载并校验摘要，在 `robot-host-tools` 中缓存工具；默认下载失败后使用系统 curl 的 HTTP/1.1 重试。后续构建复用缓存，错误版本或错误摘要会中止配置。代理使用当前构建进程的环境变量。

兼容与下载回退检查使用本地 HTTP 测试服务，无需访问外网：

```text
python NPLRuntime/tests/WebRobotBuildCompatibility.py --protoc <已有宿主protoc-3.21.12>
```
