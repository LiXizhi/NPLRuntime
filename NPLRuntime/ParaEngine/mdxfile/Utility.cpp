...
// LiXizhi: moved from main to this file
				   ...
				   )
{
	char tmp[256];
	char hexdat[256];
	memset(szReturnBuffer,0,dwMaxReturnChar);
	memset(hexdat,0,256);
	DWORD i;
	if (tab_str)
		lstrcat(szReturnBuffer,tab_str);
	if (offset!=-1)
	{
		sprintf(tmp,"0x%08X",offset);
		lstrcat(szReturnBuffer,tmp);
		lstrcat(szReturnBuffer, "   ");
	}
	memset(tmp,0,sizeof(tmp));
	for (i=0 ; (i<dwMaxReturnChar) && (i<dwUnknowSize);++i)
	{
		sprintf(hexdat,"%02X ",((unsigned char*) lpUnknow)[i]);
		lstrcat(szReturnBuffer,hexdat);
		memset(hexdat,0,256);
		tmp[i%colunm_number] = ( (isprint(((unsigned char*) lpUnknow)[i] == 0)) || (((unsigned char*) lpUnknow)[i] == '\0') || (((unsigned char*) lpUnknow)[i] == '\t') || (((unsigned char*) lpUnknow)[i] == '\n') || (((unsigned char*) lpUnknow)[i] == '\r') ) ? ( '.' ) : ( ((unsigned char*) lpUnknow)[i] );
		if ( (i+1) % colunm_number == 0)
		{
			lstrcat(szReturnBuffer, "   ");
			lstrcat(szReturnBuffer, tmp);
			lstrcat(szReturnBuffer,"\r\n");
	...
	for (DWORD j=i % colunm_number; j  <colunm_number;++j)
		strcat(szReturnBuffer, "   ");
	strcat(szReturnBuffer, "   ");
	strcat(szReturnBuffer, tmp);
}
...
BOOL utilShortenPath(char *path, char *retbuf, int maxpathsize)
{
	BOOL b; 
	char *buf1,*buf2;
	buf1 =(char*)calloc(sizeof(char), (lstrlen(path) +10));
	lstrcpy(buf1,path);
	buf2 =(char*)malloc(sizeof(char) * maxpathsize);
	if (buf1[lstrlen(buf1)]!='\\')
	{
		buf1[lstrlen(buf1)]='\\';
		b=PathCompactPathEx(buf2,buf1, maxpathsize,'\\');
		utilRemBkSlash(buf2,buf2,maxpathsize);
	}
	else
	{
		b=PathCompactPathEx(buf2,buf1, maxpathsize,'\\');
	...
	int a=0, // tmp char count
		b=0, // number of char excep '\\'
		c=0; // number of '\\'
	if (lstrlen(path)==0)
		return false;
	int i;
