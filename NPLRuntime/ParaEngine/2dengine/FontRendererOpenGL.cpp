...
bool ParaEngine::CFontRendererOpenGL::updateTextWithoutDraw(const char16_t* strText, const RECT& rect, DWORD dwTextFormat)
{
...
int ParaEngine::CFontRendererOpenGL::GetLineHeight(const char16_t* strText, const RECT& rect, DWORD dwTextFormat)
{
	if (updateTextWithoutDraw(strText, rect, dwTextFormat))
	{
		return (int)this->_commonLineHeight;
	}
	else
	{
		return -1;
...
bool ParaEngine::CFontRendererOpenGL::GetLettersInfo(std::vector<GLLabel::LetterInfo>*& lettersInfo, int*& horizontalKernings, const char16_t* strText, const RECT& rect, DWORD dwTextFormat)
{
	if (updateTextWithoutDraw(strText, rect, dwTextFormat))
	{
		lettersInfo = &_lettersInfo;
		horizontalKernings = _horizontalKernings;
		return true;
	}
	else
	{
