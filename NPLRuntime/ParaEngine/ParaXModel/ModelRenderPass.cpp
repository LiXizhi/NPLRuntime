...
bool ModelRenderPass::init(CParaXModel *m, SceneState* pSceneState)
{
	...
	//if(opacity != -1)
	//{
	//	/*GETD3D(CGlobals::GetRenderDevice())->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_BLENDFACTORALPHA );
	//	GETD3D(CGlobals::GetRenderDevice())->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );
	//	GETD3D(CGlobals::GetRenderDevice())->SetTextureStageState( 0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE );
	//	
	...
	//	// GETD3D(CGlobals::GetRenderDevice())->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_DIFFUSE );
	//}
	...
	if(is_rigid_body)
	{
		...
		mat = mat1 * CGlobals::GetWorldMatrixStack().SafeGetTop();
		CGlobals::GetWorldMatrixStack().push(mat);
		GETD3D(CGlobals::GetRenderDevice())->SetTransform(D3DTS_WORLD, mat.GetConstPointer());
	}
#endif
	return true;
}
