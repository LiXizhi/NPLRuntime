...
int CBipedObject::InstallFields(CAttributeClass* pClass, bool bOverride)
{
	...
	// Joint/Constraint attribute fields
	pClass->AddField("JointCount", FieldType_Int, NULL, (void*)GetJointCount_s, NULL, "number of joints created by this biped", bOverride);
	pClass->AddField("CreateJoint", FieldType_String, (void*)CreateJointStr_s, NULL, NULL, "create joint from config string: {Type=1,OtherObject=\"name\",PivotAX=0,...}", bOverride);
	pClass->AddField("JointProperty", FieldType_String, (void*)SetJointPropertyByIndex_s, (void*)GetJointPropertyByIndex_s, NULL, "set/get joint property by index: {Index=0,...}", bOverride);
	pClass->AddField("ReleaseJoint", FieldType_Int, (void*)ReleaseJointByIndex_s, NULL, NULL, "release joint by index", bOverride);
	pClass->AddField("ReleaseAllJoints", FieldType_void, (void*)ReleaseAllJoints_s, NULL, NULL, "release all joints", bOverride);

	// Vehicle attribute fields
	...
	pClass->AddField("ReleaseVehicle", FieldType_void, (void*)ReleaseVehicle_s, NULL, NULL, "release the vehicle", bOverride);
...
IParaPhysicsConstraint* CBipedObject::CreateJoint(int constraintType, CBipedObject* pOtherObject,
	...
	const Vector3& axisInA, const Vector3& axisInB)
{
	...
	if (pOtherObject)
	{
		if (!pOtherObject->m_dynamicPhysicsActor)
		{
			pOtherObject->LoadDynamicPhysics();
		}
		pActorB = pOtherObject->m_dynamicPhysicsActor;
	}

	IParaPhysicsConstraint* pConstraint = CGlobals::GetPhysicsWorld()->CreateConstraint(
		constraintType, m_dynamicPhysicsActor, pActorB,
		pivotInA, axisInA, pivotInB, axisInB, true);

	if (pConstraint)
	{
		pConstraint->SetUserData(this);
	}

	return pConstraint;
}

void CBipedObject::ReleaseJoint(IParaPhysicsConstraint* pConstraint)
{
	if (!pConstraint)
		return;

	CGlobals::GetPhysicsWorld()->ReleaseConstraint(pConstraint);
}

void CBipedObject::ReleaseAllJoints()
{
	if (m_dynamicPhysicsActor)
	{
		CGlobals::GetPhysicsWorld()->ReleaseConstraintsByActor(m_dynamicPhysicsActor);
	}
}

void CBipedObject::SetJointProperty(IParaPhysicsConstraint* pConstraint, const char* property)
{
	if (pConstraint)
	{
		CGlobals::GetPhysicsWorld()->SetConstraintProperty(pConstraint, property);
	}
}

const char* CBipedObject::GetJointProperty(IParaPhysicsConstraint* pConstraint)
{
	if (pConstraint)
	{
		return CGlobals::GetPhysicsWorld()->GetConstraintProperty(pConstraint);
	}
	return "";
}

int CBipedObject::GetJointCount()
{
	if (m_dynamicPhysicsActor)
	{
		return CGlobals::GetPhysicsWorld()->GetConstraintCountByActor(m_dynamicPhysicsActor);
	}
	return 0;
}

IParaPhysicsConstraint* CBipedObject::GetJoint(int index)
{
	if (m_dynamicPhysicsActor)
	{
		return CGlobals::GetPhysicsWorld()->GetConstraintByActor(m_dynamicPhysicsActor, index);
	}
	return nullptr;
}
...
bool CBipedObject::CreateVehicle()
{
	...
	IParaPhysicsVehicle* pVehicle = CGlobals::GetPhysicsWorld()->CreateVehicle(m_dynamicPhysicsActor);
	if (pVehicle)
	{
		pVehicle->SetUserData(this);
	}
	return pVehicle != nullptr;
}

void CBipedObject::ReleaseVehicle()
{
	if (m_dynamicPhysicsActor)
	{
		CGlobals::GetPhysicsWorld()->ReleaseVehicleByActor(m_dynamicPhysicsActor);
	}
}

bool CBipedObject::HasVehicle()
{
	if (m_dynamicPhysicsActor)
	{
		return CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) != nullptr;
	}
	return false;
}

int CBipedObject::AddWheel(const char* wheelConfig)
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
	if (!pVehicle || !wheelConfig)
		return -1;

	return CGlobals::GetPhysicsWorld()->AddWheelToVehicle(pVehicle, wheelConfig);
}

int CBipedObject::GetWheelCount()
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
...
void CBipedObject::SetWheelSteering(int wheelIndex, float steering)
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
	if (pVehicle)
	{
		pVehicle->SetSteeringValue(steering, wheelIndex);
	}
}

void CBipedObject::SetWheelEngineForce(int wheelIndex, float force)
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
	if (pVehicle)
...
void CBipedObject::SetWheelBrake(int wheelIndex, float brake)
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
	if (pVehicle)
	{
		pVehicle->SetBrake(brake, wheelIndex);
	}
}

float CBipedObject::GetVehicleSpeed()
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
...
Matrix4* CBipedObject::GetWheelTransform(int wheelIndex, Matrix4& outMatrix)
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
	if (pVehicle)
	{
		PARAMATRIX paraMatrix;
		pVehicle->GetWheelTransform(wheelIndex, &paraMatrix);
		memcpy(&outMatrix, &paraMatrix, sizeof(Matrix4));
		return &outMatrix;
	}
	return nullptr;
}

void CBipedObject::ResetVehicleSuspension()
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
	if (pVehicle)
...
//////////////////////////////////////////////////////////////////////////
// String-based Attribute Methods Implementation
//////////////////////////////////////////////////////////////////////////

void CBipedObject::CreateJointStr(const char* config)
{
	if (!config)
		return;
	...
	CBipedObject* pOtherObject = nullptr;
	if (msg["OtherObject"].GetType() == NPL::NPLObjectBase::NPLObjectType_String)
	{
		...
		if (!otherName.empty())
		{
			...
			{
				pOtherObject = static_cast<CBipedObject*>(pObj);
			}
		}
	}

	IParaPhysicsConstraint* pConstraint = CreateJoint(constraintType, pOtherObject, pivotInA, pivotInB, axisInA, axisInB);
	
	// Apply additional properties from the same config
	if (pConstraint)
	{
		SetJointProperty(pConstraint, config);
	}
}

void CBipedObject::SetJointPropertyByIndex(const char* config)
{
	if (!config)
		return;
	...
	m_nSelectedJointIndex = index;

	IParaPhysicsConstraint* pConstraint = GetJoint(index);
	if (pConstraint)
	{
		SetJointProperty(pConstraint, config);
	}
...
const char* CBipedObject::GetJointPropertyByIndex()
{
	IParaPhysicsConstraint* pConstraint = GetJoint(m_nSelectedJointIndex);
	if (pConstraint)
	{
		return GetJointProperty(pConstraint);
	}
	return "";
}

void CBipedObject::ReleaseJointByIndex(int index)
{
	IParaPhysicsConstraint* pConstraint = GetJoint(index);
	if (pConstraint)
	{
		ReleaseJoint(pConstraint);
	}
}

void CBipedObject::SetVehicleControlStr(const char* config)
{
	IParaPhysicsVehicle* pVehicle = m_dynamicPhysicsActor ? 
		CGlobals::GetPhysicsWorld()->GetVehicleByActor(m_dynamicPhysicsActor) : nullptr;
	if (!pVehicle || !config)
...
const char* CBipedObject::GetVehicleState()
{
	static std::string sCode;
	NPL::NPLObjectProxy msg;

	msg["HasVehicle"] = HasVehicle();
	msg["WheelCount"] = (double)GetWheelCount();
	msg["Speed"] = (double)GetVehicleSpeed();
