...
if NOT EXIST "bin\boost" (
	cd bin
	powershell -Command "Invoke-WebRequest http://dl.bintray.com/boostorg/release/1.64.0/source/boost_1_64_0.zip -OutFile boost.zip"
	7z x boost.zip -obin > nul
...
rem call "..\cmake\bin\cmake.exe" ../../Client/
rem msbuild  %~dp0\bin\client_win32\CLIENT.sln /verbosity:minimal /property:Configuration=Release
